...
class PairstradingStrategy: 
    ...
    def generate_signals_ml(
        ...
        for t in range(len(self.df_trading["zscore"])):
            ...
            # récuperer de la probabilité pour ce jour 
            if ml_mode!= 'none' and signals.index[t] in ml_probas.index :
                ml_proba=ml_probas.loc[signals.index[t]]
            else :
                ml_proba=1.0
            
            if ml_mode=='filter':
                ml_confident=ml_proba> ml_threshold
            else:
                ml_confident=True
            ...
            # Position ouverte
                ...
                elif ml_mode =='filter' and ml_proba< 0.3 :  # ML detecte un breakout : sortir 
                    position=0
                    print(f" ML Exit ( Breakout risk ): {signals.index[t].date()} z={z:.2f}")
                
            signals.iloc[t, signals.columns.get_loc("positions")] = position

            # mode sizing : Ajuster la taille des positions :
            if ml_mode=='sizing':
                signals['ml_proba']= signals.index.map(lambda x : ml_probas.loc[x] if x in ml_probas.index else 0.5)
                signals['position_size']=signals['positions']*signals['ml_proba']
            else:
                signals['position_size']=signals['positions']
        ...
        # statistiques :
        print(f"{'='*60}")
        print(f"Signaux géneres avec ML")
        print(f"{'='*60}")
        print(f"ML Mode : {ml_mode}")
        if ml_mode =='filter':
            print(f" ML Threshold : {ml_threshold}")
        
        num_trades=(self.signals['positions'].diff()!=0).sum()
        days_in_position=(self.signals['positions']!=0).sum()

        print(f" le nombre des trades : {num_trades}")
        print(f" Les jours en position : {days_in_position}")
        print(f"Long spread {(self.signals["positions"]==1).sum()} jours")

        print(f" Short spread {(self.signals["positions"]==-1).sum()} jours")

        if ml_mode != 'none':
            ml_filtered=(self.signals['positions']!=0).sum()
            print(f" Trades avec ML filter : {ml_filtered}")
    ...
    def run_full_analysis(
        ...
        # 6. Signaux
        self.generate_signals_ml(entry_threshold, exit_threshold, stop_loss, ml_mode=ml_mode,ml_threshold=ml_threshold)
    ...
    def breakout_ml(self): 

        
        features=pd.DataFrame({
            'z_score': self.df_trading['zscore'],
            'z_velocity':self.df_trading['zscore'].diff(),
            'spread_volatility': self.df_trading['zscore'].rolling(20).std(),
            'beta_change': self.df_trading['beta_kalman'].diff(),
            'z_abs':abs(self.df_trading['zscore'])
        })
        target=(self.df_trading['zscore'].shift(-15).abs()<0.5).astype(int)

        features= features.dropna()
        target=target[features.index]
        # slipt 80% train / 20% test
        x_train,x_test,y_train,y_test=train_test_split(features,target, test_size=0.2, shuffle=False)
        # entraîner votre model 
        model=RandomForestClassifier(n_estimators=100,max_depth=5,min_samples_leaf=50,random_state=42)
        model.fit(x_train,y_train)
        # prédiction sur tout dataset :
        ...
        self.ml_model=model
        self.ml_predictions=pd.Series(probas,index=features.index)
        return pd.Series(probas , index=features.index)

    

    def backtest_fast (self, transaction_cost:float=0.0001, initial_capital:float=100000): 
    ...
    # Backtest de la stratégie
        ...
        returns_clean = returns_net.dropna()
        pnl_clean = pnl_cumulative.dropna()
        ...
        for key,value in metricssimple.items():
            if 'Return' in key :
    ...
    def run_fast_analysis(
        ...
        # 7. Backtest
        metricssimple = self.backtest_fast(transaction_cost, initial_capital)
